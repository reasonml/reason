...
let copy_mapper = fun
  ...
  let module R = Migrate_parsetree_413_412_migrate in
  {
    To.Ast_mapper.
    (*$ foreach_field (fun s ->
        printf
    ...
    *)
    ...
    cases = (fun _ x -> List.map copy_case (cases mapper (List.map R.copy_case x)));
    ...
    expr = (fun _ x -> copy_expression (expr mapper (R.copy_expression x)));
    ...
    pat = (fun _ x -> copy_pattern (pat mapper (R.copy_pattern x)));
    ...
    typ = (fun _ x -> copy_core_type (typ mapper (R.copy_core_type x)));
