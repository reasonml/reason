...
module Make_witness (Ast : Ast) = struct
  type types =
    < (*$ foreach_type (fun m s -> printf "%-21s : Ast.%s.%s;\n" s m s) *)
    out_value : Ast.Outcometree.out_value
    ; out_type : Ast.Outcometree.out_type
...
let compare_ocaml_version
      ...
      (type out_type1)
      (type out_type2)
      (type out_class_type1)
      (type out_class_type2)
      (type out_module_type1)
      (type out_module_type2)
      (type out_sig_item1)
      (type out_sig_item2)
      (type out_type_extension1)
      (type out_type_extension2)
      (type out_phrase1)
      (type out_phrase2)
      (*$*)
        ((module A) :
      ...
      ((module B) :
        ...
        (*$*) >
          ocaml_version) : (A.types, B.types) type_comparison
  =
...
type ('from, 'to_) migration_functions =
  ...
  ; copy_out_type_extension :
      'from get_out_type_extension -> 'to_ get_out_type_extension
...
let migration_compose
      (ab : ('a, 'b) migration_functions)
      (bc : ('b, 'c) migration_functions) : ('a, 'c) migration_functions
  =
  { (*$
      foreach_type (fun _ s ->
  ...
  ; copy_out_module_type =
      compose bc.copy_out_module_type ab.copy_out_module_type
  ; copy_out_sig_item = compose bc.copy_out_sig_item ab.copy_out_sig_item
  ; copy_out_type_extension =
      compose bc.copy_out_type_extension ab.copy_out_type_extension
...
module type Migrate_module = sig
  ...
  *)
  ...
  val copy_out_class_type :
     From.Outcometree.out_class_type
    -> To.Outcometree.out_class_type

  val copy_out_module_type :
     From.Outcometree.out_module_type
    -> To.Outcometree.out_module_type

  val copy_out_sig_item :
     From.Outcometree.out_sig_item
    -> To.Outcometree.out_sig_item

  val copy_out_type_extension :
     From.Outcometree.out_type_extension
    -> To.Outcometree.out_type_extension
...
let migrate
      ...
      ((module B) :
        ...
        ; out_phrase : out_phrase2
        (*$*) >
          ocaml_version) : (A.types, B.types) migration_functions
  =
  match A.Version with
...
module Convert (A : OCaml_version) (B : OCaml_version) = struct
  let { (*$ foreach_type (fun _ s -> printf "copy_%s;\n" s) *)
        copy_out_value
      ; copy_out_type
      ; copy_out_class_type
      ; copy_out_module_type
...
(*$
  foreach_version (fun suffix version ->
    ...
    printf "end\n";
    printf
      "let ocaml_%s : OCaml_%s.types ocaml_version = (module OCaml_%s)\n"
      suffix
      suffix
      suffix)
*)
...
module OCaml_56 = struct
  module Ast = Ast_56
  include Make_witness (Ast_56)

  let version = 560
  let string_version = "5.6"
end

let ocaml_56 : OCaml_56.types ocaml_version = (module OCaml_56)
(*$*)
...
*)
include
  Register_migration (OCaml_408) (OCaml_409) (Migrate_parsetree_408_409)
    (Migrate_parsetree_409_408)

include
  Register_migration (OCaml_409) (OCaml_410) (Migrate_parsetree_409_410)
    (Migrate_parsetree_410_409)

include
  Register_migration (OCaml_410) (OCaml_411) (Migrate_parsetree_410_411)
    (Migrate_parsetree_411_410)

include
  Register_migration (OCaml_411) (OCaml_412) (Migrate_parsetree_411_412)
    (Migrate_parsetree_412_411)

include
  Register_migration (OCaml_412) (OCaml_413) (Migrate_parsetree_412_413)
    (Migrate_parsetree_413_412)

include
  Register_migration (OCaml_413) (OCaml_414) (Migrate_parsetree_413_414)
    (Migrate_parsetree_414_413)

include
  Register_migration (OCaml_414) (OCaml_500) (Migrate_parsetree_414_500)
    (Migrate_parsetree_500_414)

include
  Register_migration (OCaml_500) (OCaml_51) (Migrate_parsetree_500_51)
    (Migrate_parsetree_51_500)

include
  Register_migration (OCaml_51) (OCaml_52) (Migrate_parsetree_51_52)
    (Migrate_parsetree_52_51)

include
  Register_migration (OCaml_52) (OCaml_53) (Migrate_parsetree_52_53)
    (Migrate_parsetree_53_52)

include
  Register_migration (OCaml_53) (OCaml_54) (Migrate_parsetree_53_54)
    (Migrate_parsetree_54_53)

include
  Register_migration (OCaml_54) (OCaml_55) (Migrate_parsetree_54_55)
    (Migrate_parsetree_55_54)

include
  Register_migration (OCaml_55) (OCaml_56) (Migrate_parsetree_55_56)
    (Migrate_parsetree_56_55)
(*$*)
