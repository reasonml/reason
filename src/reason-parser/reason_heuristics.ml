...
let bsExprCanBeUncurried expr =
  match Ast_408.Parsetree.(expr.pexp_desc) with
  | Pexp_fun _
  | Pexp_apply _ -> true
  | _ -> false
...
let isPipeFirst e = match Ast_408.Parsetree.(e.pexp_desc) with
...
let isPipeFirstWithNonSimpleJSXChild e = match Ast_408.Parsetree.(e.pexp_desc) with
  | Pexp_apply(
    {pexp_desc = Pexp_ident({txt = Longident.Lident("|.")})},
