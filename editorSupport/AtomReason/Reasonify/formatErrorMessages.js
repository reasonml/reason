...
var renderSourcePreview = function (diagnosticType, previewExpand, fileText, range) {
  ...
  if (previewExpand == -1) {
    ...
    var indent = repeat(' ', config.indentCode);
...
var reasonifyTypeLines = function(mlType, width) {
  try {
    return Reasonify.reasonifyTypeLines(mlType, width, config);
  } catch (e) {
...
var reasonifySignatureItemLines = function(mlType, width) {
  try {
    var processedItems = Reasonify.reasonifyManySignatureItems([mlType], width, config);
...
var humanMessage = function(str) {
  return repeat(' ', config.indentHuman) + bold("> " + str);
};
...
var codeMessage = function(str) {
  return str.split('\n').map(function(s) {
    return repeat(' ', config.indentCode) + s;
  }).join('\n');
};
...
var renderHelpSection = function(resolution) {
  var wrapTo = Math.round(4 * config.columns / 5);
...
var renderDiffTableLines = function(inferredLines, expectedLines) {
  ...
  var maxColumnWidth = config.columns / 2 - centerLine;
...
var KindPrinters = {
  ...
  'TypeErrors.SignatureMismatch': function(diagnostic) {
    ...
    var niceInferredSignatureItem =
      diagnostic.commonMLData.details.inferredValueType && Reasonify.niceifyType(diagnostic.commonMLData.details.inferredValueType, config);
    var niceExpectedSignatureItem =
      diagnostic.commonMLData.details.expectedValueType && Reasonify.niceifyType(diagnostic.commonMLData.details.expectedValueType, config);
    var wrapAtColumnWidth = Math.round(config.columns / 2) - 4;
  ...
  'TypeErrors.IncompatibleType': function(diagnostic) {
    ...
    for (var i = 0; i < Math.max(inferredEquivalentTypes.length, expectedEquivalentTypes.length); i++) {
      ...
      var addedExpectedLines =
        niceExpectedEquivalentType ?
          reasonifyTypeLines(niceExpectedEquivalentType, Math.round(config.columns / 2) - 4) : [];

      if (i == 0 ||
         (niceInferredEquivalentType &&
          niceInferredEquivalentType.trim() !== Reasonify.niceifyType(inferredEquivalentTypes[0].trim(), config) ||
          niceExpectedEquivalentType &&
          niceExpectedEquivalentType.trim() !== Reasonify.niceifyType(expectedEquivalentTypes[0], config).trim())) {
        if (i > 0) {
    ...
    for (var i = 0; i < conflictPairs.length; i++) {
      ...
      var addedInferredLines = inferredConflictEquivalentTypes ?
        joinArrays(
          Reasonify.reasonifyEquivalentTypeLines(
            Reasonify.niceifyTypes(inferredConflictEquivalentTypes, config),
            Math.round(config.columns / 2) - 4,
            config
          ),
          '  =',
          config
      ...
      var addedExpectedLines = 
        expectedConflictEquivalentTypes ?
          joinArrays(
            Reasonify.reasonifyEquivalentTypeLines(
              Reasonify.niceifyTypes(expectedConflictEquivalentTypes, config),
              Math.round(config.columns / 2) - 4,
              config
            ),
            '  =',
    ...
    if (diagnostic.commonMLData.details.existentialMessage) {
      lines.push('');
      var niceExistential = Reasonify.niceifyType(diagnostic.commonMLData.details.existentialMessage, config);
  ...
  'TypeErrors.MismatchTypeArguments': function(diagnostic) {
    ...
    if (observedCount < expectedCount) {
      lines.push(
        humanMessage("You have supplied " + error(observedCount) + " type arguments to " + Reasonify.niceifyType(typeConstructor, config) + " - but it expects " + success(expectedCount))
      );
    } else {
      lines.push(
        humanMessage("You have supplied " + error(observedCount) + " type arguments to " + Reasonify.niceifyType(typeConstructor, config) + " - but it only expects " + success(expectedCount))
      );
    }
    var resolution = {
      intro:null,
  ...
  'TypeErrors.NotAFunction': function(diagnostic) {
    ...
    var reType = reasonifyType(Reasonify.niceifyType(type, config), Math.round(config.columns / 2), config);
    lines.push('');
  ...
  'TypeErrors.RecordFieldNotInExpression': function(diagnostic) {
    ...
    var reExpressionType = reasonifyType(Reasonify.niceifyType(expressionType, config), Math.round(config.columns / 2), config);
    var reBelongToType = reasonifyType(Reasonify.niceifyType(belongToType, config), Math.round(config.columns / 2), config);
  ...
  'TypeErrors.RecordFieldError': function(diagnostic) {
    ...
    var reRecordType = reasonifyType(Reasonify.niceifyType(recordType, config), Math.round(config.columns / 2), config);
    var reBelongToType = reasonifyType(Reasonify.niceifyType(belongToType, config), Math.round(config.columns / 2), config);
  ...
  'TypeErrors.UnboundConstructor': function(diagnostic) {
    var lines = [];
    var namespacedConstructor = Reasonify.niceifyType(diagnostic.commonMLData.details.namespacedConstructor, config);
  ...
  'TypeErrors.UnboundRecordField': function(diagnostic) {
    var lines = [];
    var fieldName = Reasonify.niceifyType(diagnostic.commonMLData.details.fieldName, config);
    var mainMessage =
  ...
  try {
    ...
    for (var i = 0; i < errors.length; i++) {
      ...
      if (diagnostic.scope === 'file') {
        if (config.showFileHeaders) {
          lines.push(renderFileHeader(diagnostic));
        }
        if (diagnostic.commonMLData) {
          var previewExpand = diagnostic.type === 'ERROR' ? config.errorPreviewExpand : config.warningPreviewExpand;
          var sourcePreview =
