...
if something then (
  if somethingElse then () else "blah"
);

/* These parens should be kept around just the last if/then*/
if something then (if somethingElse then ())
else "blah";
...
if true then (
  if true then print_string "one"
  else print_string "two"
);

/* Should print two */
if true then (
  if false then print_string "one"
  else print_string "two"
);
...
if printIfFirstArgGreater then
  fun a b => (
  ...
  fun a b =>
    if a > b then print_string "b < a"
    else print_string "a <= b"
);

/* Doesnt print if second arg is greater/less than (depending on printIfFirstArgGreater */
/* This is parsed/printed correctly, but not intuitively! */
if printIfFirstArgGreater then
  fun a b => (
    if a > b then print_string "a > b"
    else (
...
if printIfFirstArgGreater then
  fun a b => (if a > b then print_string "a > b")
else (
  fun a b => if a > b then print_string "b < a"
);

/* Should be printed as:
  if printIfFirstArgGreater then fun a b => (if a > b then print_string "a > b") else (
    fun a b => if a > b then print_string "b < a"
   );
    */
if something then (if somethingElse then ());
...
/* Wait why semicolon? */
/* Because SugarML is a functional */
if 10 < 100 then
...
/* To push *one* and only *one* item to the front of a list - use :: */
...
let rec size soFar lst => switch lst {
  | [] => 0
...
let myList = 1::2::3::myList;

/*CommentAfterConsBeforeAppendedTo */
...
let howDoLongMultiBarPatternsWrap x => switch x {
...
let letsPutAWhereClauseOnTheLast x => switch x {
  | AnotherReallyLongVariantName _ _ _
  | AnotherReallyLongVariantName2 _ _ _ => 0
  | ReallyLongVariantName {
      someField,
 ...
 * to avoid conflicts with named arguments.
...
let explictlyPassed = optional ?a: None ?b: None;

let a = None;

let explictlyPassed = optional ?a ?b: None;
...
*/
/* Some easy concessions: What really matters? What's most important to push?
It's not the little things like comment formats. It's convictions such as
[everything should evaluate to something].
   00. [module X] is *actually* [let module X = ..] If `let` "exports" at the
   top level for values, why doesn't it for submodules. It's already kind of
   strange how we make a special case for `type x = `.. (It still exports, but
   doesn't use the `let` keyword).
   01. [module type X] is *actually* [let module type X].
   0. Typle types look like tuples. Why don't list types look like lists?

       type intTuples = (int, int);
       type listOfString = [string];

   0.4 Make comments more familiar.
   0.5 Fix refs, !, :=, = simultaneously.

       1.
          "We make mutation uglier"
          var x = blah;
          x.val = x.val + 10;
       ...
       2. This frees up ! which should represent not!
       3. = should still represent asignment in the case of refs.
       4. = should *only* represent assignment. Polymorphic compare must be
        referenced via `compare`.

   1. Single quotes
   2. Line based comments supported too.
   3. Two forms of functions.

      Form One: Capable of creating very familiar looking function structure,
      but is actually the same curried powerhouse as every other form. When we
      open with `fn/function`, we know that the last item should be an
      expression. (There's probably some reduce/reduce conflicts here since the
      context may not be enough to infer if we're at the last item yet. We
      might need to parse the items backwards.
      --------------------------------------------------------------------------
        ...
        Example: Super familiar.

          Requires that the last item be a `semi_terminated_seq_expr`, which
          *can* be distinguished from a record pattern.

                                   /-------- pattern ------------\  /expr\
          let addPoints = function (pointOne:point, pointTwo:point) {
            let x1 = pointOne.x;
            let x2 = pointTwo.x;
            x1 + x2;
          };
          let result = addPoints({x:0}, {x:1});

        Example Two: But you can curry!

          let addPoints = function (pointOne: point, pointTwo:point) delta {
            pointOne.x + pointTwo.x + delta;
          };
          let result = addPoints({x:0}, {x:0}) 10;

      Form Two: No `function` keyword, but arrow indicates where returned
      expression is: This is the same as ES6 lambdas.
      --------------------------------------------------------------------------

                          /-------- pattern ----------\   /arrow\ /expr\
          let addPoints = (pointOne:point, pointTwo:point)   =>   {
            let x1 = pointOne.x;
            let x2 = pointTwo.x;
            x1 + x2;
          };
        ...
        Example Two: And you can still curry!
        ...
        Example Three: This is the form that has super slick currying:

          let addPoints (pointOne:point, pointTwo:point) delta =>
            pointOne.x + pointTwo.x + delta;
      ...
      var myFunction = fun (a, b) => {
        var aa = a;
        var bb = b;
        aa + bb;
      }

      // Then it's pretty clear that if you just have syntactic sugar
      var myFunction = fun (a, b) {
        var aa = a;
        var bb = b;
        aa + bb;
      ...
      (then you say you can optionally write it in super concise + curried
      mode):
      let myFunction a b => {
        let aa = a;
        let bb = b;
...
/**
 * Every star or forward slash after the first char must be escaped.
