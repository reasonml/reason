...
module Sibling = {
  let createElement = (~foo=?, ~children: list(component), ()) => {
    displayName: "test",
  };
};
...
module Namespace = {
  module Foo = {
    let createElement =
        (~intended=?, ~anotherOptional as x=100, ~children, ()) => {
      displayName: "test",
    };
  };
};
...
let div = (~argument1=?, ~children, ()) => {displayName: "test"};
...
let notReallyJSX = (~foo, ~bar, children) => {displayName: "test"};
...
let asd = [@foo] <One test=true foo=2> "a" "b" </One>;
let asd2 =
  [@foo]
  [@JSX]
  One.createElementobvioustypo(~test=false, ~children=["a", "b"], ());

let span = (~test: bool, ~foo: int, ~children, ()) => 1;
let asd = [@foo] <span test=true foo=2> "a" "b" </span>;
 ...
 */
<Foo a=10 b=0> <Bar /> <Bar /> <Bar /> <Bar /> </Foo>;
