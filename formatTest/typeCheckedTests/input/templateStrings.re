...
let singleLineConstant = ` Single line template `;
let singleLineInterpolate = ` Single line ${addTwo(1, 2)} ! `;
...
let templteWithAttribute = [@attrHere] `
...
let resultPrintTwo = printTwo("short one", `
  Passing line template
  Passing line template
  Passing line template
  Passing line template
...
let singleLineInterpolateWithEscapeTick = ` Single \`line ${addTwo(1, 2)} ! `;

let singleLineConstantWithEscapeDollar = ` Single \${line template `;

// The backslash here is a backslash literal.
let singleLineInterpolateWithBackslashThenDollar = ` Single \$line ${addTwo(2, 3)} ! `;
...
let backToBackInterpolations = `
 ...
 Before${addTwo(0, 0)}${reallyLongIdent ++ reallyLongIdent ++ reallyLongIdent ++ reallyLongIdent}After
