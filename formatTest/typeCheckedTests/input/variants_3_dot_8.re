...
type foo = [ | #foo];
type colorList3 = [ colorList2 | foo | #Red | #Black | foo];
...
let accessDeeplyWithArg =
    (
      ...
      LocalModule.AccessedThroughModuleWithTwo(_, x),
    ) => x;
...
let accessDeeplyWithArg = x =>
  ...
  | LocalModule.AccessedThroughModuleWithTwo(x as retVal1, y as retVal2) =>
    retVal1 + retVal2 + 1
  };
...
let rec accessDeeplyWithArgRecursive = (x, count) =>
  ...
  | LocalModule.AccessedThroughModuleWith(x) as entirePattern =>
    ...
    if (count > 0) {
      ...
      accessDeeplyWithArgRecursive(entirePattern, count - 1);
    }
...
let howWouldWeMatchFunctionArgs = (HeresTwoConstructorArguments(x, y)) =>
  x + y;

/* How would we annotate said arg? */
let howWouldWeMatchFunctionArgs =
    (HeresTwoConstructorArguments(x, y): combination<'wat>) =>
  x + y;

let matchingTwoCurriedConstructorsInTuple = x =>
  switch (x) {
  | (HeresTwoConstructorArguments(x, y), HeresTwoConstructorArguments(a, b)) =>
    x + y + a + b
  };
...
let rec commentPolymorphicCases: 'a. option<'a> => int =
  fun
  | Some(a) => 1
  /* Comment on one */
  | None => 0;
...
type contentHolder<'a> =
  | ContentHolderWithReallyLongNameCauseBreak('a)
  | ContentHolder('a);
